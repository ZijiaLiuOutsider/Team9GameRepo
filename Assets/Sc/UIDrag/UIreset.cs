...
public class UIreset: MonoBehaviour
{
    ...
    [SerializeField] private Transform tagetUI;//黑色图片位置信息
    ...
    //当拖着物体到达指定位置（2这者之间的相对距离小到一个程度的时候）判定到达指定位置，固定到正确位置
    {
        ...
        {
            if (Mathf.Abs(transform.position.x - tagetUI.position.x) <= 2.0f &&
                Mathf.Abs(transform.position.y - tagetUI.position.y) <= 2.0f)
            {
                // transform.position = new Vector2(tagetUI.position.x, tagetUI.position.y);
                transform.position = new Vector2(startPos.x, startPos.y);
                number = 0;
                //SceneManager.LoadScene("2-11");
                originUI.SetActive(true);
